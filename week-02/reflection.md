# Week 2 Reflection

**Student Name:** Dan Martinson

**Date Completed:** 7/26/2026

## Prompts

**What clicked for you this week?**

```
The three layers inside the computer and how they all work together.
```

**What's still confusing?**

```
Submitting my work through GitHub in an organized and professional way that is acceptable to the instructors.
```

**How does this week's material connect to a cybersecurity career path you're interested in?**

```
The different ways that each layer can be affected on the attack surface.
```

**One thing you'd tell a friend just starting this course:** Read the questions carefully.

```
Your answers need to be in complete sentences.
```

---

## Professional Growth Check

- [x] I documented my reflection clearly and in my own words

- [x] I used structured formatting in my submission

- [x] My commit message was meaningful and descriptive

---

*CyberVisionaries Institute — Cyber Foundations, Tier I*
